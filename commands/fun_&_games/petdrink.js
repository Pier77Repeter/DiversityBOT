...
module.exports = {
  ...
  async execute(client, message, args) {
    ...
    const cooldown = await cooldownManager(client, "petDrinkCooldown", this.cooldown, message.guild.id, message.author.id);

    if (cooldown == null) {
      try {
        return await message.reply("Your pet is already hydrated, try later");
      } catch (error) {
        return;
      }
    }
