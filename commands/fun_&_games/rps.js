...
module.exports = {
  ...
  async execute(client, message, args) {
    ...
    const btnRpsRock = new ButtonBuilder().setCustomId("btn-rps-btnRpsRock").setEmoji("🪨").setLabel("Rock").setStyle(ButtonStyle.Primary);
    const btnRpsPaper = new ButtonBuilder().setCustomId("btn-rps-btnRpsPaper").setEmoji("🧻").setLabel("Paper").setStyle(ButtonStyle.Primary);
    const btnRpsScissors = new ButtonBuilder().setCustomId("btn-rps-btnRpsScissors").setEmoji("✂️").setLabel("Scissors").setStyle(ButtonStyle.Primary);
    ...
    collector.on("end", async (collected, reason) => {
      if (reason === "time") {
        embed.setTitle("Rip").setDescription("Someone didn't respond in time >:(").setFooter({ text: null });
        ...
        if (authorChoice.beats === mentionedUserChoice.name) {
          ...
          embed.setTitle(`${message.author.username} 🏆`).setFooter(null);
        } else if (mentionedUserChoice.beats === authorChoice.name) {
          result = `**${mentionedMember.user.username} wins, GG!**`;
          embed.setTitle(`${mentionedMember.user.username} 🏆`).setFooter(null);
        } else {
          result = "**It's a tie!**";
          embed.setTitle("Nobody wins 🤷").setFooter(null);
        }
