...
module.exports = {
  ...
  async execute(client, message, args) {
    ...
    const cooldown = await cooldownManager(client, "petFeedCooldown", this.cooldown, message.guild.id, message.author.id);

    if (cooldown == null) {
      try {
        return await message.reply("Out of food, feed your pet later");
      } catch (error) {
        return;
      }
    }
